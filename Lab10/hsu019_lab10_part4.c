...
enum SetPasswordSM_States { SP_wait, SP_recive, SP_release, SP_check, };


int new_password[4];
int new_check[4];
unsigned char new_cnt;
unsigned char new_tmpB;
unsigned char new_inputB;

int SetPWSMTick(int state) {
    new_inputB = ~PINB & 0x80;
    unsigned char y = GetKeypadKey();
    switch (state)
    {
    case SP_wait:
        if (new_inputB == 0x80 && y == '*') {
            state = SP_recive;
        }
        break;
    case SP_recive:
    ...
    case SP_check:
        if (cnt == 5) {
            if (Check(new_password, new_check) == 1) {
                new_tmpB = 0x01;
                PORTB = new_tmpB | new_inputB;
            }
        }
        break;
    default:
